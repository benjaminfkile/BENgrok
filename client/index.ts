...
const startTunnel = async (baseUrl: string, entry: SavedProfileEntry, index: number) => {
  ...
  ws.on("open", () => {
    const logMsg = chalk.green(`✅ Tunnel Ready [${index}]: ${publicUrl} → ${entry.FriendlyName} (${entry.URL})`)
    console.log(logMsg)
    log(logMsg)
  ...
  // Receive a request from server → forward to local target → send response back
  ws.on("message", (message) => {
    const reqMsg = JSON.parse(message.toString()) as {
      method: string
      url: string
    ...
    const options: http.RequestOptions = {
      ...
      path: path.posix.join(parsedTarget.pathname || "/", reqMsg.url || "/"),
    ...
    const proxyReq = proxyRequest(options, (proxyRes) => {
      ...
      proxyRes.on("end", () => {
        ...
        const logMsg = chalk.cyan(
          `[${tunnelId}] ${reqMsg.method} ${reqMsg.url} → ${proxyRes.statusCode} (${entry.FriendlyName})`
        )
        console.log(logMsg)
        log(logMsg)
      })
    ...
    // Write request body (support base64 for binary uploads)
    if (reqMsg.isBase64Request && reqMsg.bodyBase64) {
      const reqBuf = Buffer.from(reqMsg.bodyBase64, "base64")
      proxyReq.write(reqBuf)
    } else if (reqMsg.body) {
  ...
  process.stdin.on("data", (key) => {
    ...
    if (!isNaN(num)) {
      ...
      if (tunnel) {
        ...
        console.log(chalk.magenta(`📋 Copied ${tunnel.friendlyName} URL to clipboard: ${tunnel.publicUrl}`))
      } else {
