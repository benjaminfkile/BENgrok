...
const displayTunnelList = () => {
  ...
  activeTunnels
    ...
    .forEach((tunnel) => {
      console.log(
        `  [${tunnel.index}] ${tunnel.friendlyName}: ${tunnel.publicUrl}`
      );
    });
};
  ...
  process.stdin.on("data", (key) => {
    ...
    if (!isNaN(num)) {
      ...
      if (tunnel) {
        ...
        console.log(
          chalk.magenta(
            `📋 Copied ${tunnel.friendlyName} URL to clipboard: ${tunnel.publicUrl}`
          )
        );
      } else {
  ...
  if (Object.keys(profiles).length > 0) {
    ...
    if (useProfile) {
      ...
      if (selected && profiles[selected]) {
        ...
        console.log(
          chalk.green(
            `✅ Loaded profile '${selected}' with ${entries.length} tunnel(s).`
          )
        );
      } else {
